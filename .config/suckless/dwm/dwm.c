...
runautostart(void)
{
    char autostart_sh[PATH_MAX];

    snprintf(autostart_sh, sizeof(autostart_sh),
             "%s/.dwm/autostart.sh", getenv("HOME"));

    if (access(autostart_sh, X_OK) == 0) {
        if (fork() == 0) {
            execl("/bin/sh", "/bin/sh", autostart_sh, NULL);
            exit(0);
        }
    }
}
